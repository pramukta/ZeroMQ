...
int zmq_send_c P(( int _tp1, const char * _tp2, int _tp3));
...
#endif
{
	...
	_tp0 = zmq_send_c(_tp1, _tp2, _tp3);

	res = MLAbort ?
		MLPutFunction( mlp, "Abort", 0) : MLPutInteger( mlp, _tp0);
L3: L2:	MLDisownString(mlp, _tp2);
L1: 
L0:	return res;
} /* _tr0 */


void zmq_recv_c P(( int _tp1, int _tp2));
...
int zmq_bind_c P(( int _tp1, const char * _tp2));
...
int zmq_connect_c P(( int _tp1, const char * _tp2));
...
int zmq_device_c P(( int _tp1, int _tp2, int _tp3));
...
#endif
{
	...
	_tp0 = zmq_device_c(_tp1, _tp2, _tp3);
...
int zmq_socket_c P(( int _tp1, int _tp2));
...
static struct func {
	...
	const char  *f_name;
		...
		{ 2, 0, _tr1, "zmq_recv_c" },
		{ 2, 0, _tr2, "zmq_bind_c" },
		{ 1, 0, _tr3, "zmq_close_c" },
		{ 2, 0, _tr4, "zmq_connect_c" },
		{ 3, 0, _tr5, "zmq_device_c" },
		{ 1, 0, _tr6, "zmq_init_c" },
		{ 2, 0, _tr7, "zmq_socket_c" },
		{ 1, 0, _tr8, "zmq_term_c" },
		{ 0, 0, _tr9, "zmq_version_c" }
		};
...
#endif
{
	...
	if (_res) _res = _definepattern(mlp, (char *)"ZMQSend[socket_integer, message_String, option_Integer]", (char *)"{socket, message, option}", 0);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQRecv[socket_integer, option_Integer]", (char *)"{socket, option}", 1);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQBind[socket_Integer, endpoint_String]", (char *)"{socket, endpoint}", 2);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQClose[socket_Integer]", (char *)"{socket}", 3);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQConnect[socket_Integer, endpoint_String]", (char *)"{socket, endpoint}", 4);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQDevice[device_Integer, fsocket_Integer, bsocket_Integer]", (char *)"{device, fsocket, bsocket}", 5);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQInit[nthreads_Integer]", (char *)"{nthreads}", 6);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQSocket[context_Integer, type_Integer]", (char *)"{context, type}", 7);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQTerminate[context_Integer]", (char *)"{context}", 8);
	if (_res) _res = _definepattern(mlp, (char *)"ZMQVersion[]", (char *)"{}", 9);
